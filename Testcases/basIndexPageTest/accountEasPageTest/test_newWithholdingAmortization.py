...
class TestNewWithholdingAmortization():
    ...
    def test_newWithholdingAmortization(self):
        ...
        with allure.step("选择业务类型"):
            self.newWithholdingAmortizationPage.input_operationType('预提')
        with allure.step("输入备注"):
            self.newWithholdingAmortizationPage.input_boeAbstract('测试预提申请单(新)')

        with allure.step("选择客商"):
            self.newWithholdingAmortizationPage.input_vendor('hc供应商1')
        ...
        with allure.step("输入预提类型"):
            self.newWithholdingAmortizationPage.input_costOperationSubType('预提1')
        with allure.step("选择责任部门"):
            self.newWithholdingAmortizationPage.selectCostExpenseDept('AD', deptName='A部门')
        with allure.step("输入项目"):
            self.newWithholdingAmortizationPage.input_costProject('hc项目1')
        with allure.step("输入金额"):
            self.newWithholdingAmortizationPage.input_costExpenseAmount('100.10')
        with allure.step("选择开始时间"):
            self.newWithholdingAmortizationPage.select_costBeginDateStr('2020-2-1')
        with allure.step("输入期限（月）"):
            self.newWithholdingAmortizationPage.input_costTheTerm('3')
        with allure.step("选择贷方科目"):
            self.newWithholdingAmortizationPage.input_voucherAccount('测试')
